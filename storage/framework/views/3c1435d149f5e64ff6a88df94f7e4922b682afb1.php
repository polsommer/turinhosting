    ...
    $('#configTokenBtn').on('click', function (event) {
        $.ajax({
            ...
            swal({
                type: 'success',
                title: 'Token created.',
                text: '<p>To auto-configure your node run the following command:<br /><small><pre>cd /etc/jexactyl && sudo wings configure --panel-url <?php echo e(config('app.url')); ?> --token ' + data.token + ' --node ' + data.node + '<?php echo e(config('app.debug') ? ' --allow-insecure' : ''); ?></pre></small></p>',
                html: true
            })
        }).fail(function () {
            swal({
